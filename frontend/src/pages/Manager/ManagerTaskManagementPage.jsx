...
const ManagerAllTasksManagementPage = () => {
  ...
  // 2. state to hold user list
  const [users, setUsers] = useState([]);

  // 3. call getAllUserMutation on mount
  useEffect(() => {
    getAllUserMutation();
  }, [getAllUserMutation]);

  // 4. update local users state when users fetched
  useEffect(() => {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 font-sans">
      <h2 className="text-3xl font-bold text-center text-gray-800 mb-10">All Tasks (Manager View)</h2>
              ...
              {editingTaskId === task._id ? (
                ...
                <>
                  ...
                  <p className="text-gray-600 mb-1">
                    ...
                    {createdByUser ? `${createdByUser.name} (${createdByUser.email})` : "Unknown"}
                  </p>
                  <p className="text-gray-600 mb-1">
                    <strong>Assigned to:</strong>{" "}
                    {assignedUser ? `${assignedUser.name} (${assignedUser.email})` : "Unknown"}
                  </p>
                  <p className="mb-1">
